...
public class BoardController {
	...
	// 생성자가 없어지면 @autowired 필요
	
	// ------------------------------------------------------------------------------------------------------
	
	// 컨트롤러의 모든 메소드 => JoinPoint(조인포인트)
	// 로그찍는 어드바이스를 실제로 적용하고자 하는 메소드 => PointCut
	// 항상 조인포인트는 포인트컷의 크기보다 같거나 크다. (포인트컷은 조인포인트의 부분집합이라 보면 됨)
	...
	public String edit(int boardNo, Model model) {
		model.addAttribute("board", boardService.findBoardByNo(boardNo));   // borad라는 이름으로 검색결과를 가져온 후에 board/edit로 forward 하겠다
		return "board/edit";
	}
