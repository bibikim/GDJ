...
public class Main {
	...
	public static void m2() {
		...
		// Tree맵은 TreeMap만 사용할 수 있는 메소드가 다수 있으므로
		// TreeMap 타입으로 생성성하는 것이 좋다.
		...
		// 오름차순 정렬 ↔ 내림차순 정렬 (정렬을 왼오,오왼 바꾼다라고 생각하면 됨)
		NavigableMap<Integer, String> map2 = map.descendingMap();
		for(Map.Entry<Integer, String> entry : map2.entrySet()) {
			System.out.println(entry.getKey() + ":" + entry.getValue());
		}
		
		// 다시 descendingMap을 호출하면 오름차순 정렬이 된다.
		NavigableMap<Integer, String> map3 = map2.descendingMap();
		for(Integer key : map3.keySet()) {	// key값만 빼오기
			System.out.println(key + ":" + map3.get(key));		
		}
		// 위에 두 코드는 같은 것임! 추천은 위에거
