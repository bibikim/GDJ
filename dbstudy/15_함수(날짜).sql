...
SELECT
     ...
     , EXTRACT(TIMEZONE_HOUR FROM SYSTIMESTAMP) AS 시  -- 우선적으로 TIMEZONE 설정 필요
...
-- 4. 날짜 연산
--    1) 하루(1일, 24시간)를 숫자 1로 처리
--       (12시간을 숫자 0.5로 처리)
--    2) 특정 단위 후 날짜
--       (1) 1년 후   : 12개월 후 계산(ADD_MONTHS 함수 사용), 별도의 함수 없음
...
SELECT
       SYSDATE - 1 AS 어제
     , SYSDATE + 1 AS 내일
     , SYSDATE - 0.5 AS "12시간전"  -- 칼럼이름에 숫자로 시작은 못하니까 ""를 넣어주면 가능
     , SYSDATE + 0.5 AS "12시간후"
     , TO_CHAR(SYSDATE - 0.5, 'MM/DD AM HH:MI:SS') AS "12시간전"
     , TO_CHAR(SYSDATE + 0.5, 'MM/DD PM HH:MI:SS') AS "12시간후"
...
--    ex) 3개월 지나면 비밀번호 변경하게끔 구현하는 경우에 사용 가능
SELECT
