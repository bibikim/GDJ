...
-- 프로시저 작성
-- 학과번호(DEPTNO) 전달하면 소속 학생, 교수, 학과 모두 제거하는 프로시저    > DEPARTMENT는 제일 나중에 제거(PK). DEPTNO을 참조하는 애들 먼저 삭제
-- 프로시저 실행은 학과번호=101 학과를 대상으로 실행
-- 프로시저 이름은 MY_PROC
-- 예외 발생 시 예외코드 출력 + 예외메시지 출력 + ROLLBACK

CREATE OR REPLACE PROCEDURE MY_PROC(DNO IN DEPARTMENT.DEPTNO%TYPE)  -- 타입을 직접 적어줘도 됨. NUMBER
IS
BEGIN
    DELETE FROM STUDENT WHERE DEPTNO = DNO;     -- 변수DNO와 DEPTNO가 같을 때 DELETE 
...
EXECUTE MY_PROC(101);  -- 101이 DNO로 전달되어 들어가서 프로시저 실행



-- 사용자 함수
-- 학생 아이디를 전달하면 해당 학생의 학번을 반환하는 함수 만들기
-- 아이디 'jun123'을 전달하면 학번 '20101'이 반환되는지 확인
-- 함수 이름 GET_STUDENT
CREATE OR REPLACE FUNCTION GET_STUDENT(UID STUDENT.USERID%TYPE)
RETURN NUMBER  -- 반환하는 학번의 타입이 NUMBER(5) 사이즈 무시 가넝
IS
    SNO STUDENT.STUDNO%TYPE; -- 전달하는건 ID, 반환은 학번. 따라서 변수선언 필요
BEGIN 
    SELECT STUDNO
      INTO SNO
      FROM STUDENT
     WHERE USERID = UID;     -- Dals
    RETURN SNO;              -- STUNO를 SNO에 저장시켜 놨으니까 반환해서 보려는 것도 SNO. 따라서 RETURN SNO 
END GET_STUDENT;
