/*
    사용자 함수                              <--> 내장함수. 이미 만들어져 있는 함수 UPPER, LOWER 등등..
    
    1. Function
    2. 어떤 값을 반환할 때 사용
    3. 사용자가 만든 함수의 개념
    4. 값을 반환할 때 RETURN을 사용
    5. 입력 파라미터 사용 가능(출력 파라미터 필요 X 사용 안 함. RETURN이 있기 때문)
    6. 값을 확인할 수 있도록 SELECT문과 같은 쿼리문에서 호출
    7. 형식
        CREATE OR REPLACE FUNCTION 함수_이름[(매개변수)]    -- 매개변수: 입력 파라미터
        RETURN 반환타입
        IS
            변수선언
        BEGIN
...
-- 함수 FUNC1 호출(호출은 함수결과를 확인하거나 사용할 수 있는 쿼리문에서 확인. 주로 SELECT문에서 함)
SELECT FUNC1()
  FROM DUAL;
...
-- 함수 FUNC2 정의
SELECT FUNC2(EMPLOYEE_ID)    -- 모든 사원의 FULLNAME을 호출
  FROM EMPLOYEES;
  

-- 함수 FUNC3 정의
-- 연봉을 전달하면 각각 'A, B, C그룹'을 반환하는 함수
-- 15000 이상: A그룹
-- 8000 이상  : B그룹
-- 나머지     : C그룹
CREATE OR REPLACE FUNCTION FUNC3(SAL EMPLOYEES.SALARY%TYPE)
RETURN VARCHAR2   -- 반환할 값의 타입을 적어주기
IS
    SAL_GROUP VARCHAR2(10 BYTE);
...
-- 함수 FUNC3 호출
SELECT EMPLOYEE_ID, FIRST_NAME, SALARY, FUNC3(SALARY)
  FROM EMPLOYEES;
