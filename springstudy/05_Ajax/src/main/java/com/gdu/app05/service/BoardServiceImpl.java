	...
	*/
	...
	public ResponseEntity<Board> execute1(HttpServletRequest request) {
		...
		if(title.isEmpty()) {
			...
			entity = new ResponseEntity<Board>(board, HttpStatus.OK);  // 200 성공(제목을 입력한 경우)하면 board를 만들어서 entity로!
		}											// $.ajax()의 success에서 처리
		
		return entity;
	}

	@Override    // board/detail2
	public ResponseEntity<Board> execute2(String title, String content) {
		
		ResponseEntity<Board> entity = null;
		HttpHeaders header = new HttpHeaders();
		header.add("Content-type", MediaType.APPLICATION_JSON_VALUE);   // application/json 타입으로 UTF-8로 가져오겠다!
