...
function fn_checkOne() {
	...
	$('.check_one').click(function(){
		// 체크 상태 변경
		let checkCount = 0;
		for(let i = 0; i < $('.check_one').length; i++) { // $(this)는 클릭 당한애 1개만 해당, 따라서 그냥 클래스째로 length 달아주기
			// 체크 몇 개 되어 있는건지 갯수 확인
			checkCount += $($('.check_one')[i]).prop('checked');    // 인덱스를 붙이면 자바스크립트 일반변수이기 때문에 $()한번 더 붙여야 prpo 사용 가능. 
